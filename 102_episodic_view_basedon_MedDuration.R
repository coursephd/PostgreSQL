...
epsd01chk <- all_met_rmsd04 [, .(epsdur = max(studyday) - min (studyday) + 1), by = .(mr_no, Code02, Code, description, eps011, diffeps, releps011, maxdurstd)]

# calculate the median after removing events with only 1 day
all_met_rmsd05 <- epsd01chk [epsdur > 1]
all_met_rmsd05 <- all_met_rmsd05 [, `:=` (epsmedian = median(epsdur, na.rm= FALSE) ), by = .(Code02, eps011)]
all_met_rmsd05 <- all_met_rmsd05 [, pattype := ifelse(epsdur <= maxdurstd + 30, "01Compliant", "02Non-compliant"), ]


all_met_rmsd06 <- merge (x = all_met_rmsd04,
                         y = all_met_rmsd05,
                         by = c("mr_no", "Code02", "Code", "description", "eps011", "diffeps", "releps011", "epsdur", "maxdurstd"),
                         all.x = TRUE)
...
fwrite(all_met_rmsd06, "D:/Hospital_data/ProgresSQL/analysis/102_episodicdis01_medDuration.csv")
