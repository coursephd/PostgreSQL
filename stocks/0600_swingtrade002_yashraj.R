...
all02 <- all02 [, mfi09reg := MFI (price.close, volume, n = 9), by = .(ticker)]
...
atr_n <- 20
atr_dn = as.data.table( ATR(all02 [ , c("price.high", "price.low", "price.close"), ], n = atr_n) )
atr_dn <- atr_dn [, allrow := .I, ]


all02 <- Reduce(function(...) merge(..., by = c("allrow"), all=T),  
                list( all02, bb_dn, atr_dn ) )

write_feather(all02, "D:/My-Shares/analysis/0600_1d_data_stin.feather")
...
system(paste('"c:/Program Files/Python310/python.exe"', 'D:/My-Shares/prgm/swing_trade/0600_yh_stock_part02_tf_supertrend_trial_feather02.py'))

all03 <- as.data.table( arrow::read_feather("D:/My-Shares/analysis/0600_1d_data_stout.feather") )

########################################################
#
# Remove first 40 rows to avoid incorrect calculations
#
########################################################
all03 <- all03 [ nrow > bb_n * 2]



all02 <- all02 [, filter001 := ifelse( shift(mfi09, type = c("lag"), n = 1) >= 99 & mfi09 < 99 &
                                       shift(mfi09avg, type = c("lag"), n = 1) >= 99 & mfi09avg < 99, 1, 0), by = .(ticker)]

all02 <- all02 [, filter002 := ifelse( shift(mfi09, type = c("lag"), n = 1) >= 99 & shift(mfi09, type = c("lag"), n = 2) >= 99 &
                                       shift(mfi09, type = c("lag"), n = 3) >= 99 & shift(mfi09, type = c("lag"), n = 4) >= 99 &
                                       shift(mfi09avg, type = c("lag"), n = 1) >= 99 & shift(mfi09avg, type = c("lag"), n = 1) >= 99 &
                                       shift(mfi09avg, type = c("lag"), n = 3) >= 99 & shift(mfi09avg, type = c("lag"), n = 4) >= 99, 1, 0), by = .(ticker)]

output <- all02 [filter001 == 1]
output02 <- all02 [filter002 == 1]
