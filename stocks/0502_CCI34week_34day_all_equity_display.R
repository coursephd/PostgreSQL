...
a04all <- a03all [, qudrant := case_when(jdk_rs55 > 100 & jdk_momratio55 > 100 ~ 1,
...
a05all <- a05all [, `:=`(rule001 = ifelse( cci_34w > 100 & cci_34d > 125 & (cci_34d > cci_34w) & volume > 100000, 1, 0), 
                         rule002 = ifelse( cci_34d < -100, 1, 0),
                         ema20 = ema(price.close, 20), 
                         ema50 = ema(price.close, 50),
                         ema200 = ema(price.close, 200),
                         ema5 = ema(price.close, 5), ema13 = ema(price.close, 13),
                         ema21 = ema(price.close, 21), ema55 = ema(price.close, 55),
                         cci34d_gt100 = ifelse( cci_34d > 100, 1, 0),
...
a05all <- a05all [, `:=`(overalltrnd = ifelse(price.close >= ema20 & ema20 >= ema50 & ema50 >= ema200, 1, 0 ) ), ]
...
# of recent past of high of CCI34week and CCI34days >= 100 (in last 25 days)
#
# If the days are more than say 5 then do not take the trade as a breakout trade
#
#############################################################################################

a05all <- a05all [, `:=` (rule001sum = Reduce(`+`, shift(rule001, 0:24)), 
                          rule002sum = Reduce(`+`, shift(rule001, 0:24)) ), by =.(ticker)]

a06all <- a05all [ overalltrnd == 1 & rule001sum == 1 & rule002sum == 1 ]
...
# Check how many stocks appear on each day
a06all_chk <- a06all [, .(n = uniqueN(ticker),
                            stocks = paste(ticker, collapse = ",", sep = "") ), by = .(ref.date, qudrant)]

# Based on the following page, the heatmap of trades is generated
# https://towardsdatascience.com/time-series-calendar-heatmaps-9f576578fcfe

a06all_chk <- a06all_chk [, weekday := as.POSIXlt(ref.date)$wday, ]
a06all_chk <- a06all_chk [, weekdayf := factor(weekday,levels=rev(1:7),labels=rev(c("Mon","Tue","Wed","Thu","Fri","Sat","Sun")),ordered=TRUE) , ]
a06all_chk <- a06all_chk [, monthf := factor(month(ref.date),levels=as.character(1:12),labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"),ordered=TRUE) , ]
a06all_chk <- a06all_chk [, yearmonth := factor(as.yearmon(ref.date)), ]
a06all_chk <- a06all_chk [, week := as.numeric(format(ref.date,"%W")), ]
a06all_chk <- a06all_chk [, monthweek := 1+week-min(week), by =.(yearmonth) ]

a06all_chk <- a06all_chk [, ntrademonth := sum(n), by = .(yearmonth)]

p <- ggplot(a06all_chk [ qudrant ==1 ], aes(monthweek, weekdayf, fill = n)) + 
  geom_tile(colour = "white") + 
  facet_grid(year(ref.date) ~ monthf) + 
  scale_fill_gradient(low="red", high="green") + 
  xlab("Week of Month") + ylab("") + 
...
#########################################################
#
# nimblr concept EMA convergence / divergence
# https://twitter.com/Deishma/status/1073194800176873472/photo/1
#
# 5 EMA > 21 EMA > 55 EMA 
# CCI Daily > 100
# BH > 50% CH Candle
# Ratio = (ema5 - ema21)/(ema21 - ema55) needs to be between 0.382 & 2.272 Fibo number. 
# We rounded it to 0.38 - 2.28
#
#########################################################

ema001 <- a05all [, emaratio := round( (ema5 - ema21) / (ema21 - ema55), 1),   ]
ema001 <- ema001 [, emaratio_cat := ifelse(emaratio >= 0.38 & emaratio <= 2.28, 1, 0),  ]
ema001 <- ema001 [, nimb001 := ifelse(overalltrnd == 1 & emaratio_cat == 1 & bh_ch_cat == "Breakout" & cci_34d > 100, 1, 0), ]


ema001 <- a05all [, c("allrow", "ref.date", "price.open", "price.high", "price.low", "price.close", "price.adjusted", 
                      "volume", "jdk_rs55", "jdk_momratio55", "qudrant", "ch", "bh", "bh_ch_cat",
                      "nrow")]
