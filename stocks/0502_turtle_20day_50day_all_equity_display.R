...
a04all <- a04all [, `:=` (ema21 = ema(price.close, 21),
                          ema55 = ema(price.close, 55),
                          ema200 = ema(price.close, 200),
                          smavol20 = sma(volume, 20),
                          max21 = roll_max(price.close, 21),
                          max34 = roll_max(price.close, 34),
...
a04all <- a04all [, `:=`(above200ema = ifelse(price.close >= ema200, "Above_200ema", "Below_200ema"),
                         ...
                         above21max = ifelse(price.close >= max21, "Above_21", ""),
                         above34max = ifelse(price.close >= max34, "Above_34", ""),
                         above55max02 = ifelse(price.close >= max55, 1, 0),
                         above21max02 = ifelse(price.close >= max21, 1, 0),
                         above34max02 = ifelse(price.close >= max34, 1, 0),
                         overalltrnd = ifelse(price.close >= ema21 & ema21 >= ema55 & ema55 >= ema200, 1, 0 ) ), ]
...
a04all <- a04all [, `:=` (above55max02sum = Reduce(`+`, shift(above55max02, 0:24)), 
                          above21max02sum = Reduce(`+`, shift(above21max02, 0:24)),
                          above34max02sum = Reduce(`+`, shift(above34max02, 0:24))), by =.(ticker)]
...
turtle001_21 <- a04all [ overalltrnd == 1 & above21max02 == 1 & above21max02sum == 1 ]
turtle001_34 <- a04all [ overalltrnd == 1 & above34max02 == 1 & above34max02sum == 1 ]
turtle001_50 <- a04all [ overalltrnd == 1 & above55max02 == 1 & above55max02sum == 1 ]

turtle001_all <- rbindlist( list(turtle001_21, turtle001_34, turtle001_50), idcol ="Condition")
...
fwrite(turtle002, "D:\\My-Shares\\analysis\\0502_turtle_21_34_50day_all_equity_display.csv")
