...
ami003 <- ami003 [, z_wgt_year := z5 + z10 + z20 + z30 + z40 + z60 + z120 + z180 + z240, ]
...
ami003 <- ami003 [, z_wgt_std_year := ifelse(z_wgt_year >= 0, 1 + z_wgt_year, 1/(1 - z_wgt_year) ), ]
...
ami003 <- ami003 [, z_rank_short := 1:.N, by =.(ref.date)]

ami003 <- ami003 [ order(ref.date, -z_wgt_std_inter), ]
ami003 <- ami003 [, z_rank_inter := 1:.N, by =.(ref.date)]

ami003 <- ami003 [ order(ref.date, -z_wgt_std_year), ]
ami003 <- ami003 [, z_rank_year := 1:.N, by =.(ref.date)]

ami004 <- dcast(data = ami003 [ ref.date >= "2021-05-01"], # & ref.date >= "2020-09-01"],
                z_rank_inter ~ ref.date,
                value.var = c("ticker"))

ami005 <- ami004 [z_rank_inter <= 15] # [z_rank_short <= 15 & z_rank_inter <= 15]
