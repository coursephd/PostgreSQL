...
maxscr <- unq03comb[, .(maxscr = max(a01jac) ), by = .(mr_no, totrow, alldis, refday2, combdis)]
maxscr_t <- dcast (data = maxscr, 
                   mr_no + totrow + alldis ~ refday2, 
                   value.var = c("maxscr", "combdis"))

maxscr02 <- maxscr [, .(scr = uniqueN(mr_no)), by = .(maxscr, refday2)]
maxscr02_t <- dcast(data = maxscr02, 
                    maxscr ~ refday2,
                    value.var = c("scr"))


maxscr03 <- maxscr [, .(scr = uniqueN(mr_no)), by = .(maxscr, combdis, refday2)]
